...
clean: ## Clean build artifacts
	...
	rm -f npm/claude-mcp-rs npm/claude-mcp-rs.exe

install: build-release ## Install to system (requires sudo on Unix)
	@echo "Installing claude-mcp-rs..."
	@cp target/release/claude-mcp-rs /usr/local/bin/ || echo "Failed. Try: sudo make install"
...
npm-install: npm-pack ## Install npm package locally for testing
	npm install -g npm/missdeer-claude-mcp-rs-*.tgz
