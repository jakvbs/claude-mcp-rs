...
├── src/
│   ├── claude.rs               # Claude CLI wrapper implementation
│   ├── lib.rs                  # Library root
...
- **src/server.rs**: Defines the `ClaudeServer` struct and implements the `claude` tool using rmcp macros
- **src/claude.rs**: Handles spawning the Claude CLI process and parsing its JSON output
...
-- **npm/claude-mcp-rs[.exe]**: Extracted binary
